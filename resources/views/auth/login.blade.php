...
@section('content')
    <div class="flex justify-center">
        <div class="w-3/12 bg-white p-6 rounded-lg">
            @if(session('status'))
                <div class="text-white bg-red-500 p-4 rounded-lg text-center mb-6">
                    {{ session('status') }}
                </div>
            @endif
            <form action="{{ route('login') }}" method="POST">
                ...
                <div>
                    <button type="submit" class="bg-blue-500 text-white px-4 py-3 w-full font-medium rounded">Login</button>
                </div>
